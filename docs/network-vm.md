...
The built-in network and VM drivers are registered core adapters. Each ships a
strict manifest under `privileged-adapters/` and a mandatory behavior contract
that pins its lifecycle plans and enforcement decisions. New drivers can be
proposed by PR, but their implementation must be reviewed source code bound
through `defineNetworkAdapter` or `defineVmAdapter`; a project file or runtime
plugin cannot load or replace privileged code. The release bundle records the
adapter's capabilities, permissions, platform constraints, provenance, and
signing identity.

The `custom` network and VM drivers are also registry entries, but they are
marked non-trusted user-configured argv wrappers because the actual command
arrays come from the user's config.
