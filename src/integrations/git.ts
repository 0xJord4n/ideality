...
export function renderGitIncludes(
  ...
  for (const [id, identity] of Object.entries(config.identities).sort(([a], [b]) =>
    a.localeCompare(b),
  )) {
    if (!identity.git) {
      continue;
    ...
    identities[id] = [
      ...
      ...(identity.git.gpgSign === undefined
        ? []
        : ["[commit]", `\tgpgSign = ${identity.git.gpgSign ? "true" : "false"}`]),
      ...(identity.git.sshKey
        ? [
            "[core]",
            `\tsshCommand = ${gitValue(
...
export async function installGitIntegration(
  ...
  if (!registered) {
    ...
    if (add.exitCode !== 0) {
      throw new Error(`Failed to register Git config: ${add.stderr.toString().trim()}`);
    }
  }
